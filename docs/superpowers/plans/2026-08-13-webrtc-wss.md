...
event_route[xhttp:request] {
    ...
    # Bu xlog/xhttp_reply YALNIZCA yukaridaki DIS kontrol (port, metod,
    # Upgrade/Connection basliklari) tutmadiginda calisir ve o durumlar icin
    # gercekten faydali bir teshistir. ws_handle_handshake() KENDISI
    # reddederse (ornegin Sec-WebSocket-Protocol: sip eksik ya da yanlissa)
    # fonksiyon 0 doner; Kamailio bu donusu exit gibi ele alir ve script
    # if(...) icinde durur - asagidaki xlog/xhttp_reply o durumda HIC
    # CALISMAZ. Modul seviyesindeki bu red websocket modulunun KENDI log
    # satirinda (debug=2 ile) gorunur, bu route'ta degil.
...
`nginx`'in `Sec-WebSocket-Protocol` basligini aynen iletmesi yine de zorunludur
(bkz. Task 2, `web/nginx.conf`); iletmezse handshake modul seviyesinde reddedilir
ve sebebi bu route'ta degil, websocket modulunun kendi logunda gorunur.
